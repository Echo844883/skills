...
- **CSV 导入导出**：往来单位、税率、发票类型、发票号都在列里，坏行会被跳过
  并报出行号，不会因为一行有问题整份失败
- **导入发票 (PDF)**：从中国电子发票（普通/专用）的 PDF 原始文件里提取发票号码、
  开票日期、买卖双方、价税合计、税率这类有固定标签的字段，自动预填「记一笔」
  表单——保存前必须自己核对一遍，识别不出来或看着像猜的字段会在「识别结果」
  面板里标出来。只认文字版 PDF（税务系统生成的原始文件，文字能选中复制），
  扫描件/照片存的 PDF 没有文字可提取，那种请用「手动粘贴发票文字」——把 PDF
  里能复制的文字粘过来，走同一套解析逻辑；纯图片的发票只能手填表单，这个工具
  不做图像 OCR（中文发票 OCR 准确率有限，与其做一个不可靠的自动识别，不如让
  你自己确认）。在「税务参数」里填上本方的纳税人识别号，能自动判断这笔发票该
  记成收入还是支出。
...
│   │                     测算、CSV、发票 PDF 文字解析 —— 不碰 DOM，也不含任何
│   │                     写死的"现行政策"数字
│   ├── storage.js       本地版存储：localStorage 读写
│   ├── hosted-store.js  托管版存储：artifact 能力自发布 + downloads 能力导出
│   ├── app.js           渲染与事件绑定，两版共用
│   └── vendor/          第三方库（目前只有 pdf.js，用于导入发票），运行时
│                          不发起任何外部请求；见该目录下的 README 了解怎么升级
└── test/
...
51 个单元测试，覆盖价税合计拆分的舍入一致性、增值税小规模/一般纳税人两种算法、
留抵结转、企业所得税分档速算、扣除限额提示、CSV 往返与坏行处理、税务参数校验、
发票 PDF 文字解析（字段提取、买卖双方配对、方向判定、多档税率、空文本退化）。
需要 Node 18+（内置 `node:test`，没有测试框架依赖）。

浏览器端额外用 Playwright 做过端到端验证：记账、编辑、删除撤销、设置面板联动、
报表数字、CSV 导出、XSS 防护、窄屏布局；托管版的自发布循环（发布产物能重新
打开、能继续发布、设置和记录都不会在版本之间丢失、备注里的脚本标签不会截断
文档、离线/只读场景正确降级）；导入发票功能在本地版/Pages 部署版/托管版三种
形态下分别验证（手动粘贴解析、方向自动判定、pdf.js 懒加载确实懒、真实 PDF
文件走完整条提取管线无报错），以及 vendor 脚本孤岛在托管版重新发布之后依然
完整、功能依然可用。
